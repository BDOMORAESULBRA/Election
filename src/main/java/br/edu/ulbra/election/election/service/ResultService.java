...
public class ResultService {
	...
	public ResultOutput getResultByElection(Long electionId) {
		...
		ArrayList<CandidateOutput> candidatesA = new ArrayList<>();
		ArrayList<ElectionCandidateResultOutput> candidatesB = new ArrayList<>();

		Long votesValid = (long) 0, votesBlank, votesNull;

		try {
			candidatesA = candidateClientService.getListCandidatesByElectionId(electionId);
			...
			for (ElectionCandidateResultOutput c : candidatesB) {
				votesValid = votesValid + c.getTotalVotes();
			}
